...
class axi_rsp_item #(
  ...
  // Canonicalize the inactive write-response field for read transactions.
  // This value does not cause a B response; the driver controls BVALID.
  constraint c_inactive_fields {
    if (dir == AXI_READ)
      bresp == AXI_RESP_OKAY;
  }

//为什么删去了EXOKAY？？赛题给的信号端口里面没有LOCK信号
