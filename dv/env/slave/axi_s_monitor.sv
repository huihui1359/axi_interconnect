...
class axi_s_monitor #(
  ...
  extern function new(string name = "axi_s_monitor",
...
function void axi_s_monitor::clear_write_state();
...
function void axi_s_monitor::clear_read_state();
...
function axi_s_monitor::req_t axi_s_monitor::create_address_request(
  axi_dir_e dir
);
  req_t req;
...
function void axi_s_monitor::publish_write_if_complete(int unsigned id);
  ...
  while ((aw_context_by_id[id].size() != 0) &&
    ...
    if (data_context.wdata.size() != (int'(address_context.len) + 1))
      `uvm_error("AXI_S_MON_WCOUNT", $sformatf(
    ...
    if (!$cast(response_context, address_context.clone()))
      `uvm_fatal("AXI_S_MON_CLONE", "Failed to clone write ctx")
    write_response_by_id[id].push_back(response_context);
  end
endfunction

function void axi_s_monitor::capture_w_beat();
  req_t completed;
  int unsigned id;
  ...
  if (active_w_burst == null) begin
    ...
    `uvm_error("AXI_S_MON_WID", "WID changed within a W burst")
  end
  ...
  if (!$cast(completed, active_w_burst.clone()))
    `uvm_fatal("AXI_S_MON_CLONE", "Failed to clone completed W burst")
...
function void axi_s_monitor::publish_read_request();
  ...
  if (!$cast(ctx, req.clone()))
    `uvm_fatal("AXI_S_MON_CLONE", "Failed to clone read ctx")
  id = int'(req.id);
  read_context_by_id[id].push_back(ctx);
endfunction

function void axi_s_monitor::publish_b_response();
  rsp_t rsp;
  req_t completed;
  int unsigned id;
  ...
  if (write_response_by_id[id].size() == 0) begin
    `uvm_error("AXI_S_MON_B", "B response has no complete write request")
  end
  else begin
...
function void axi_s_monitor::capture_r_beat();
  rsp_t rsp;
  req_t completed;
  int unsigned id;
  ...
  if (active_r_context == null) begin
    if (read_context_by_id[id].size() == 0) begin
      `uvm_error("AXI_S_MON_R", "R burst has no pending AR ctx")
      return;
    end
    active_r_context = read_context_by_id[id][0];
  end
  else if (vif.rid !== active_r_context.id) begin
    `uvm_error("AXI_S_MON_RID", "RID changed within an R burst")
  end
  ...
  if (active_r_data.size() != (int'(active_r_context.len) + 1))
    `uvm_error("AXI_S_MON_RCOUNT", $sformatf(
...
function int unsigned axi_s_monitor::pending_count();
  int unsigned total;
...
function void axi_s_monitor::check_phase(uvm_phase phase);
  super.check_phase(phase);
  if (pending_count() != 0)
    `uvm_error("AXI_S_MON_PENDING", $sformatf(
      "Slave Monitor ended with %0d pending contexts", pending_count()))
endfunction
